...
export function EditSyllabusForm({ syllabusId }: EditSyllabusFormProps) {
  ...
  const loadSyllabusData = async () => {
    try {
      ...
      const draftSyllabi = JSON.parse(
        localStorage.getItem("syllabus-drafts") || "[]"
      );
      const allSyllabi = [...savedSyllabi, ...draftSyllabi];

      const foundSyllabus = allSyllabi.find((s: any) => s.id === syllabusId);
      if (foundSyllabus) {
        setSyllabusData(foundSyllabus);
      } else {
        // Redirect back if syllabus not found
        router.back();
      }
    } catch (error) {
  ...
  // Pass existing data to the create form component
  return <CreateSyllabusForm initialData={syllabusData} isEditing={true} />;
}
...
export function CreateSyllabusForm({
  ...
  const [syllabusData, setSyllabusData] = useState<SyllabusData>(() => {
    if (initialData) {
      return {
        ...
        textbooks: initialData.textbooks || [
        ...
        gradingScale: initialData.gradingScale || [
  ...
  const handlePolicyChange = (
    field: keyof typeof syllabusData.policies,
    value: string
  ) => {
    setSyllabusData((prev) => ({
      ...prev,
  ...
  const saveDraft = async () => {
    ...
    try {
      if (isEditing && initialData) {
        ...
        const savedSyllabi = JSON.parse(
          localStorage.getItem("syllabi") || "[]"
        );
        const draftSyllabi = JSON.parse(
          localStorage.getItem("syllabus-drafts") || "[]"
        );
        ...
        // Find and update in the correct storage
        const syllabusIndex = savedSyllabi.findIndex(
          (s: any) => s.id === initialData.id
        );
        const draftIndex = draftSyllabi.findIndex(
          (s: any) => s.id === initialData.id
        );

        if (syllabusIndex !== -1) {
          savedSyllabi[syllabusIndex] = updatedSyllabus;
          localStorage.setItem("syllabi", JSON.stringify(savedSyllabi));
        } else if (draftIndex !== -1) {
          draftSyllabi[draftIndex] = updatedSyllabus;
          localStorage.setItem("syllabus-drafts", JSON.stringify(draftSyllabi));
        }

        toast({
          title: "O'zgarishlar saqlandi",
          description: "O'quv dasturi muvaffaqiyatli yangilandi.",
        });
      } else {
        // Create new draft
        ...
        toast({
          ...
          description:
            "O'quv dasturi qoralama sifatida muvaffaqiyatli saqlandi.",
        });
      }
  ...
  const submitForReview = async () => {
    ...
    try {
      if (isEditing && initialData) {
        // Update existing and submit for review
        const savedSyllabi = JSON.parse(
          localStorage.getItem("syllabi") || "[]"
        );
        const draftSyllabi = JSON.parse(
        ...
        // Move from drafts to syllabi if needed
        const draftIndex = draftSyllabi.findIndex(
          (s: any) => s.id === initialData.id
        );
        if (draftIndex !== -1) {
          draftSyllabi.splice(draftIndex, 1);
          savedSyllabi.push(updatedSyllabus);
          localStorage.setItem("syllabus-drafts", JSON.stringify(draftSyllabi));
          localStorage.setItem("syllabi", JSON.stringify(savedSyllabi));
        } else {
          // Update in syllabi
        ...
        toast({
          ...
          description:
            "O'quv dasturi yangilanib, ko'rib chiqish uchun yuborildi.",
        });
      } else {
        // Create new and submit for review
        ...
        toast({
          title: "Ko'rib chiqishga yuborildi",
          description:
            "O'quv dasturi ko'rib chiqish uchun muvaffaqiyatli yuborildi.",
        });
      }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <Input
            ...
            onChange={(e) =>
              handleBasicInfoChange("courseCode", e.target.value)
            }
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          ...
          <Select
            ...
            onValueChange={(value) =>
              handleBasicInfoChange("credits", parseInt(value))
            }
          >
            <SelectTrigger>
              <SelectValue />
        ...
        <div>
          ...
          <Input
            ...
            onChange={(e) =>
              handleBasicInfoChange("academicYear", e.target.value)
            }
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground mb-3">
          Ushbu fan tugagandan so'ng talabalar quyidagi bilim va ko'nikmalarga
          ega bo'lishi kerak:
        </p>
      ...
      <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <Input
                    value={method.method}
                    onChange={(e) =>
                      handleAssessmentChange(index, "method", e.target.value)
                    }
        ...
        <div className="mt-4 p-3 bg-muted rounded-lg">
          <p className="text-sm font-medium">
            Jami:{" "}
            {syllabusData.assessmentMethods.reduce(
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  <Input
                    value={week.topic}
                    onChange={(e) =>
                      handleWeeklyScheduleChange(index, "topic", e.target.value)
                    }
  ...
  const currentSectionComponent =
    sections.find((s) => s.id === activeSection)?.component || renderBasicInfo;
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-2xl font-bold">
              {isEditing
                ? "O'quv dasturini tahrirlash"
                : "Yangi o'quv dasturi yaratish"}
            </h1>
            <p className="text-muted-foreground">O'qituvchi: {user?.name}</p>
          </div>
        </div>
        <div className="flex space-x-2">
          <Button variant="outline" onClick={saveDraft} disabled={isSaving}>
            <Save className="mr-2 h-4 w-4" />
            {isEditing ? "O'zgarishlarni saqlash" : "Qoralama saqlash"}
          </Button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3">
          <Card>
            ...
            <CardContent>{currentSectionComponent()}</CardContent>
          </Card>
        </div>
      </div>
    </div>
